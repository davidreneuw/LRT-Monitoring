...
def create_rot_deg(dec, inc, anc=0):
    ...
    rotate_tot = np.dot(rotate_dec, rotate_inc)
    rotate_tot = np.dot(rotate_tot, rotate_anc)
    return rotate_tot
...
def abs_value(a):
    #a is a 3D vector
    abs = np.sqrt(a[0]**2 + a[1] **2 + a[2]**2)
    return abs

def get_weather(weather, temp):
    found = False
    buffer = 0
    while (found == False):
            for i in range(len(weather)):
                if (temp - buffer <= weather[i] and temp + buffer >= weather[i]):                  
                    found = True
                    break         
            buffer = buffer + 0.2
    return i

def two_step_rotate(desired, current):
    final = np.copy(current)
    c = final[0]
    d = desired[0]
    c0 = np.array([c[0],c[1]])
    d0 = np.array([d[0],d[1]])
    angle = np.dot(d0,c0)/(np.linalg.norm(d0) * np.linalg.norm(c0))
    angle = np.arccos(angle)
    z_axis = np.array([0,0,1])
    x_axis = np.array([1,0,0])
    matrix1 = rodrigues(axis = z_axis,degree = angle)
    c = matrix1.dot(c)
    d1 = np.array([d[1],d[2]])
    c1 = np.array([c[1],c[2]])
    angle1 = np.dot(d1,c1)/(np.linalg.norm(c1) * np.linalg.norm(d1))
    matrix2 = rodrigues(axis = x_axis, degree = angle1)
    c = matrix2.dot(c)
    print(calc_accuracy(d,c))
                
def rotate(desired,current,weather= None,times=1,vector = False,return_i = False, skip = [], final = None):
    if (final is None):
        final = (np.copy(current))
    count = 0
    buffer = 0
    found = False
    #implement when weather data is finally fixed
    if (weather is not None):
        i = get_weather(weather,20)  
        if (not vector):
            d = desired[i]
        c = final[i]
    if (vector):
        ...
        c = np.array([current[:,0].mean(),current[:,1].mean(),current[:,2].mean()])
        if (vector == False):
            d = np.array([desired[:,0].mean(),desired[:,1].mean(),desired[:,2].mean()])
    """
    c = c/np.linalg.norm(c)
    d = d/np.linalg.norm(d)
    cross = np.cross(c,d)
    angle = np.dot(c,d)/(np.linalg.norm(d) * np.linalg.norm(c))
    angle = np.arccos(angle)
    matrix = rodrigues(axis = cross,degree = angle)
    """
    use = 0
    done = []
    boundaries1 = []
    ...
    for j in range(final.shape[0]):
        if (j not in skip):
            ...
            if (a > 0.00001):
                ...
                angle = np.arccos(angle)
                matrix = rodrigues(axis = cross,degree = angle)
                f = matrix.dot(final[j]/np.linalg.norm(final[j]))
                final[j] = f * np.linalg.norm(final[j])
        else:
            if (not (j-1) in skip):
                boundaries1.append(j-1)
            if (not (j+1) in skip):
                boundaries2.append(j+1)
    count = 0
    for j in skip:
        if (not (j-1) in skip and not skip.index(j) == 0):
            count = count + 1
        front = boundaries2[count]
        back = boundaries1[count]
        c1 = current[front]
        c2 = current[back]
        d1 = desired[front]
        d2 = desired[back]
        ...
        length = front - back
        weight = place/length
        c  = (c1 * weight) + c2 * (1-weight)
        d = (d1 * weight) + d2 * (1-weight)
        temp_c = np.copy(c)
        c = c/np.linalg.norm(c)   
        d = d/np.linalg.norm(d)
        cross = np.cross(c,d)
        ...
        angle = np.arccos(angle)
        matrix = rodrigues(axis = cross,degree = angle)
        final[j] = matrix.dot(final[j])
    if (return_i):
        return(np.transpose(final),i)
    return np.transpose(final)
def rodrigues(desired=None,current=None,axis = None,degree = None):
    if (desired is not None and current is not None):
        desired = desired/np.linalg.norm(desired)
        current = current/np.linalg.norm(current)
        cross = np.cross(current,desired)
        cross = cross/np.linalg.norm(cross)
        cos = (np.dot(desired,current)/(np.linalg.norm(desired) * np.linalg.norm(current)))
        sin = np.sin(np.arccos(cos))
    elif (axis is not None and degree is not None):
        cross = axis/np.linalg.norm(axis)
        cos = np.cos(degree)
        sin = np.sin(degree)
    a1 = np.array([cross[0]**2 * (1-cos) + cos,
                      cross[0] * cross[1] * (1-cos) - (sin * cross[2]),
                      cross[0] * cross[2] * (1-cos) + (sin * cross[1])])
    a2 = np.array([cross[0] * cross[1] * (1-cos) + sin * cross[2],
                      cross[1] ** 2 * (1-cos) + cos,
                      cross[1] * cross[2] * (1-cos) - sin * cross[0]])
    a3 = np.array([cross[0] * cross[2] * (1-cos) - sin * cross[1],
                      cross[1] * cross[2] * (1 - cos) + sin * cross[0],
                      cross[2] ** 2 * (1 - cos) + cos])
    m = np.array([a1,a2,a3])
    return m
...
def calc_accuracy(data1, data2):
    ...
    """
    a = np.dot(data1,data2) /(np.linalg.norm(data1) * np.linalg.norm(data2))
    if (a > 1 or a < 0):
        a = 1
    a = np.arccos(a)
    a = a * 180/ np.pi
...
def scaling(i,desired,current):
    scale = desired[:,i]/current[:,i]
    scalar = scale[0]
...
def rotate_to_abs(data1, data2, make_plot=False,
    ...
    if make_plot:
        ...
        plt.savefig("/nrn/home/NRN/dcalp/lrtOps/zhiywang/lrt/plots/LRO/2021/rotated_plot.png")
...
#guess and check is probably not the best idea for rotations
...
def main():
    ...
    parser.add_argument(
        '-dir1',
        default=(USER + '/lrt/LRO/RT1Hz/2021/'),
        help='Data to be rotated')
    parser.add_argument(
        '-dir2',
        default=(USER + '/lrtOps/git/crio-data-reduction/ottSecData/2021/'),
    ...
    if getattr(args, 'mode') == 0:
        ...
        find_best_tri_rot(data1,
                          ...
                          .001)
    '''
    ottPath = "/nrn/home/NRN/drene/crio-data-reduction/ottSecData/OTT2021288.sec"
    lrePath = "/lrt/lrt/LRE/RT1Hz/2021/LRE20210915vsec.sec"
    dat1 = pd.read_fwf(
        ottPath, names=['date', 'time', 'doy', 'x', 'y', 'z', 'f'])
    dat2 = pd.read_fwf(
        lrePath, names=['date', 'time', 'doy', 'x', 'y', 'z', 'f'])
    print(find_best_tri_rot(dat1, dat2, 0.001))
